...
import { getFullPokeWorldNumber, getPokeWorldNumber } from "../utils/index";
...
function PokeCard(props) {
  ...
  useEffect(() => {
    ...
    // 1. define the cache
    let cache = {}; //cache as obj as we are geeting data in obj

    if (localStorage.getItem("pokeworld")) {
      cache = JSON.parse(localStorage.getItem("pokeworld"));
    }
    ...
    async function fetchData() {
      ...
      try {
        const baseURL =
          ...
          getPokeWorldNumber(selectedPokemon);
        ...
        localStorage.setItem("pokeworld", JSON.stringify(cache));
      } catch (err) {
  ...
  return (
    <div className="poke-card">
      ...
      <div>
        <h4>#{getFullPokeWorldNumber(selectedPokemon)}</h4>
        <h2>{name}</h2>
      </div>
      ...
      <img
        ...
        src={"/pokemon/" + getFullPokeWorldNumber(selectedPokemon) + ".png"}
        alt={`${name}-large-img`}
      />
